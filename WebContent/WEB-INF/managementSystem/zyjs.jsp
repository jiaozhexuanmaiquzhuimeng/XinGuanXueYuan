		...
		<div class="row-fluid">
			...
			<div class="span9">
				<ol class="breadcrumb">
					...
					<li><a href="#">教务教学</a></li>
					<li class="active">专业建设</li>
				</ol>
				<div class="btn-toolbar">
					<button class="btn btn-primary" onclick="window.location.href='<%=request.getContextPath()%>/managementServlet?method=forwardPage&title=zyjs'">
						...
						<tbody>
							<c:forEach items="${pageInfo.list }" var="content" varStatus="vst">
								<tr>
									...
									<td>${content.date }</td>
									<td><a href="<%=request.getContextPath()%>/managementServlet?method=updatePage&id=${content.id }&table=td_education_major&title=zyjs" title="编辑" title="编辑"> <i
											class="fa fa-pencil"></i>
									</a> <a class="btn-delete" data-contentid="${content.id }" href="#myModal" role="button" data-toggle="modal"
				...
				<div class="pagination">
					<ul class="pagination pagination-sm">
						<c:if test="${pageInfo.hasPrev }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=zyjs&pageNo=${pageInfo.prevPage }&table=${table }&title=${title }"
							...
							<c:if test="${(pageInfo.pageNo>=1) && (pageInfo.pageNo<=6) }">
								<c:forEach begin="1" end="10" var="n">
									<c:if test="${pageInfo.pageNo == n }">
										<li class="active"><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=zyjs&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
									</c:if>
									<c:if test="${pageInfo.pageNo != n }">
										<li><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=zyjs&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
						...
						<c:if test="${pageInfo.hasNext }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=zyjs&pageNo=${pageInfo.nextPage }&table=${table }&title=${title }"
