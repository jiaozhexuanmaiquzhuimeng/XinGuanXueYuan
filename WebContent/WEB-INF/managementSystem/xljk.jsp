		...
		<div class="row-fluid">
			...
			<div class="span9">
				<ol class="breadcrumb">
					...
					<li><a href="#">学生管理</a></li>
					<li class="active">心理健康</li>
				</ol>
				<div class="btn-toolbar">
					<button class="btn btn-primary" onclick="window.location.href='<%=request.getContextPath()%>/managementServlet?method=forwardPage&title=xljk'">
					...
					<table class="table">
						...
						<tbody>
							<c:forEach items="${pageInfo.list }" var="content" varStatus="vst">
								<tr>
									<td>${content.id }</td>
									<td><a href="<%=request.getContextPath()%>/userServlet?method=contentPage&id=${content.id }&table=td_stumanage_practice" title="预览文章" target="_balnk">${content.title }</a></td>
									<td>${content.author }</td>
									<td>${content.date }</td>
									<td><a href="<%=request.getContextPath()%>/managementServlet?method=updatePage&id=${content.id }&table=td_stumanage_practice&title=xljk" title="编辑" title="编辑"> 
									<i
											class="fa fa-pencil"></i>
									</a> <a class="btn-delete" href="#myModal" role="button" data-toggle="modal"
									data-contentid="${content.id}" title="删除"> 
				...
				<div class="pagination">
					<ul class="pagination pagination-sm">
						<c:if test="${pageInfo.hasPrev }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=xljk&pageNo=${pageInfo.prevPage }&table=${table }&title=${title }"
							...
							<c:if test="${(pageInfo.pageNo>=1) && (pageInfo.pageNo<=6) }">
								<c:forEach begin="1" end="10" var="n">
									<c:if test="${pageInfo.pageNo == n }">
										<li class="active"><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=xljk&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
									</c:if>
									<c:if test="${pageInfo.pageNo != n }">
										<li><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=xljk&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
						...
						<c:if test="${pageInfo.hasNext }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=xljk&pageNo=${pageInfo.nextPage }&table=${table }&title=${title }"
