					...
					<table class="table">
						...
						<tbody>
							<c:forEach items="${pageInfo.list }" var="content" varStatus="vst">
								<tr>
									<td>${content.id }</td>
									<td><a href="<%=request.getContextPath()%>/userServlet?method=contentPage&id=${content.id }&table=td_Rmstu_employment" title="预览文章" target="_blank">${content.title }</a></td>
									<td>${content.author }</td>
									<td>${content.date }</td>
									<td><a href="<%=request.getContextPath()%>/managementServlet?method=updatePage&id=${content.id }&table=td_Rmstu_employment&title=jygz" title="编辑" title="编辑"> <i
				...
				<div class="pagination">
					<ul class="pagination pagination-sm">
						<c:if test="${pageInfo.hasPrev }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=jygz&pageNo=${pageInfo.prevPage }&table=${table }&title=${title }"
							...
							<c:if test="${(pageInfo.pageNo>=1) && (pageInfo.pageNo<=6) }">
								<c:forEach begin="1" end="10" var="n">
									<c:if test="${pageInfo.pageNo == n }">
										<li class="active"><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=jygz&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
									</c:if>
									<c:if test="${pageInfo.pageNo != n }">
										<li><a
											href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=jygz&pageNo=${n}&table=${table }&title=${title }">${n}</a></li>
						...
						<c:if test="${pageInfo.hasNext }">
							<li><a
								href="<%=request.getContextPath()%>/managementServlet?method=skipPage&jsp=jygz&pageNo=${pageInfo.nextPage }&table=${table }&title=${title }"
