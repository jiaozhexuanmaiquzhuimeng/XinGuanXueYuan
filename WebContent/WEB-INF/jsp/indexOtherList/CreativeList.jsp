			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<div class="right-list clearfix">
						...
						<ul class="list-item">
							<c:forEach items="${pageInfo.list }" var="creative">
								<li><a title="${creative.title }" target="_blank"
									href="<%=request.getContextPath()%>/userServlet?method=contentPage&id=${creative.id }&table=${table }">${creative.title }</a>
									<span class="time">${creative.date }</span></li>
							</c:forEach>
						</ul>
					...
					<nav aria-label="...">
						<ul class="pagination pagination-sm">
							<c:if test="${pageInfo.hasPrev }">
								<li><a
									href="<%=request.getContextPath()%>/creativeServlet?method=CreativePage&pageNo=${pageInfo.prevPage }&table=${table }"
								...
								<c:if test="${(pageInfo.pageNo>=1) && (pageInfo.pageNo<=6) }">
									<c:forEach begin="1" end="10" var="n">
										<c:if test="${pageInfo.pageNo == n }">
											<li class="active"><a
												href="<%=request.getContextPath()%>/creativeServlet?method=CreativePage&pageNo=${n}&table=${table }">${n}</a></li>
										</c:if>
										<c:if test="${pageInfo.pageNo != n }">
											<li><a
												href="<%=request.getContextPath()%>/creativeServlet?method=CreativePage&pageNo=${n}&table=${table }">${n}</a></li>
							...
							<c:if test="${pageInfo.hasNext }">
								<li><a
									href="<%=request.getContextPath()%>/creativeServlet?method=CreativePage&pageNo=${pageInfo.nextPage }&table=${table }"
